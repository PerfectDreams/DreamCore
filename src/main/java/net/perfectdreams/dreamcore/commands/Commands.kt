...
import net.perfectdreams.dreamcore.utils.extensions.execute
...
class Commands(val m: DreamCore) {
	fun createDreamCoreCommand() = command("dreamcore", plugin = m) {
		...
		command("reload") {
			tabComplete {
				m.dreamScriptManager.scriptsFolder.listFiles().filter { it.extension == "kt" || it.extension == "kts" }.map { it.name }.filter { it.startsWith(this.args.last(), true) }.toMutableList()
			}

			execute<CommandSender> {
				val fileName = args.getOrNull(0) ?: throw CommandException("§4kk eae men cadê o script?")
		...
		command("plreload") {
			tabComplete {
				Bukkit.getPluginManager().plugins.map { it.name }.filter { it.startsWith(this.args.last(), true) }.toMutableList()
			}
			execute<CommandSender> {
				val pluginName = args.getOrNull(0) ?: throw CommandException("§4kk eae men cadê o nome do plugin?")
				val branch = args.getOrNull(1) ?: "development"
